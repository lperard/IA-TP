...
%Cas numéro 1
negamax(J,Situation,P,P,[nil,Val]) :-
	heuristique(J, Situation, Val).

%Cas numéro 2
negamax(J,Situation,_,_, [_,Val]) :-
...
meilleur([Coup], Coup).
meilleur([[Coup,Val]|T], [Coup, Val]) :-
	meilleur(T, [_,Best_Val]),
	Val < Best_Val.
meilleur([[_,Val]|T], [Best_Coup, Best_Val]) :-
	meilleur(T, [Best_Coup,Best_Val]),
	Val >= Best_Val.
test_meilleur(M):-
	L = [[a,150], [b,16], [c, 0], [d, 5], [e,0]],
	meilleur(L, M).
	/******************
  	PROGRAMME PRINCIPAL
  	*******************/
