...
colonne(C,M) :-
    nth1(A,M,_),
    findall(Elem , (nth1(_,M,L), nth1(A,L,Elem)) , C).
...
alignement_gagnant([H|T], J) :- ground(H), H=J, alignement_gagnant(T,J). 

alignement_perdant([],_).
alignement_perdant([H|T], J) :- ground(H), H\=J, alignement_perdant(T,J).
	...
	*******************************/
     ...
     /*Il faut definir quelle op�ration subit une matrice M 
    representant la situation courante
...
heuristique(J,Situation,H) :- 	% cas 3
	...
	length(List1, N1),
	adversaire(J,J2),
	findall(Alignement, (alignement(Alignement, Situation),possible(Alignement, J2)), List2),
	length(List2, N2),
	H is (N1-N2).
