...
heuristique(U,H) :-
   heuristique1(U, H1),  % au debut on utilise l'heuristique 1 
   heuristique2(U, H2),  % ensuite utilisez plutot l'heuristique 2  
   H is H1 + H2.
...
elementsDif([],[],0).
elementsDif([T1|Q1],[T2|Q2], H) :-
   (T1 = vide ->
   elementsDif(Q1,Q2,H);
   (T1 = T2 ->
   elementsDif(Q1,Q2,H);
   elementsDif(Q1,Q2,H1),
   H is H1+1)).

matriceDif([],[],0).
matriceDif([L1|R1],[L2|R2], H) :-
   elementsDif(L1,L2,H1),
   matriceDif(R1,R2,H2),
   H is H1 + H2.

heuristique1(U, H) :-
   final_state(Final),
   matriceDif(U,Final,H).
...
heuristique2Tile(U,H) :-
   final_state(Final),
   coordonnees([L1,C1],U,P),
   coordonnees([L2,C2],Final,P),
   (P \= vide -> H is abs(L1-L2)+abs(C1-C2)).
...
heuristique2(U, H) :-
   findall(H1, heuristique2Tile(U,H1), L),
   sumList(L,H).